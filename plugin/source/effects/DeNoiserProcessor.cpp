...
void DeNoiserProcessor::setLearning(bool learning){
    if(learning){
        ...
        if(isLearning){
            for(auto& channel : channels){
                if(channel.noiseProfileSamples > 0){
                    for(int i = 0; i < channel.noiseProfile.size(); i++){
                        float& bin = channel.noiseProfile[i];
                        bin /= (float)channel.noiseProfileSamples;
                    }
                }
            }
        }
...
void DeNoiserProcessor::processFrame(ChannelState& state){
    ...
    //Using curly brackets to separate it so I can use variable names without worrying if they are repeated later on
    {
        ...
        if(learning){
            state.noiseProfile[0] += magnitude0;
            state.noiseProfile[numBins - 1] += magnitude1024;
        }else{
        ...
        //Gathering stuff for visualizer storage (per channel)
    ...
    for(int i = 1; i < (numBins-1); i++){
        ...
        if(learning){
            ...
            state.noiseProfile[i] += magnitude;
        }else{
...
void DeNoiserProcessor::updateVisualizer() {
    ...
    //Smoothing (applied to the averaged data)
    for (int i = 0; i < numBins; ++i)
    {
