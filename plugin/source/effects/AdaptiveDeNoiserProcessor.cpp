...
void AdaptiveDeNoiserProcessor::processFrame(){
    ...
    frameAmplitude = std::sqrt(frameAmplitude / (float)inputBuffer.size());
    ...
    //Using curly brackets to separate it so I can use variable names without worrying if they are repeated later on
    {
        ...
        if(learning){
            noiseProfile[0] = (noiseProfile[0] * (1.0f - learningRate)) + (magnitude0 * learningRate);
            noiseProfile[numBins - 1] = (noiseProfile[numBins - 1] * (1.0f - learningRate)) + (magnitude1024 * learningRate);
        }
    ...
    for(int i = 1; i < (numBins-1); i++){
        ...
        if(learning){
            ...
            noiseProfile[i] = (noiseProfile[i] * (1.0f - learningRate)) + (magnitude * learningRate);
        }
